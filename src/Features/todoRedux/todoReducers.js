  ...
  reducers: {
    handelData: (state, payload) => {
      if (state.edit) {
        ...
        state.data = [
          ...state.data,
          { name: payload.payload.name, lastname: payload.payload.lastname, id:Math.random()*100000 },
        ];
        state.show=true;
    ...
    removePersnoe: (state, Rid) => {
      state.data = state.data.filter(
        (persone) => persone.id !== Rid.payload
      );
      state.show=true;
...
export const { handelData, removePersnoe, updatePersnoe,hideMessage } =
  todo.actions;
